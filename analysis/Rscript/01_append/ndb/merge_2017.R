...
### join
df_ndb_o_2017 <- df_ndb_o_2017_in %>% 
  full_join(df_ndb_o_2017_out, by = c("code_receipt", "name", "code_shusai", "generic", "form", "unit")) %>% 
  full_join(df_ndb_o_2017_hos, by = c("code_receipt", "name", "code_shusai", "generic", "form", "unit")) %>% 
...
### delete
rm(list = c("df_ndb_o_2017_in", "df_ndb_o_2017_out", "df_ndb_o_2017_hos"))
...
### join
df_ndb_p_2017 <- df_ndb_p_2017_in %>% 
  full_join(df_ndb_p_2017_out, by = c("code_receipt", "name", "code_shusai", "generic", "form", "unit")) %>% 
  full_join(df_ndb_p_2017_hos, by = c("code_receipt", "name", "code_shusai", "generic", "form", "unit")) %>% 
  mutate(total = if_else(is.na(total_in), 0, total_in) + 
           if_else(is.na(total_out), 0, total_out) + 
           if_else(is.na(total_hos), 0, total_hos)) %>% 
  mutate(price = ifelse(!is.na(price_in), price_in, 
...
### delete
rm(list = c("df_ndb_p_2017_in", "df_ndb_p_2017_out", "df_ndb_p_2017_hos"))
...
### join
df_ndb_t_2017 <- df_ndb_t_2017_in %>% 
  full_join(df_ndb_t_2017_out, by = c("code_receipt", "name", "code_shusai", "generic", "form", "unit")) %>% 
  full_join(df_ndb_t_2017_hos, by = c("code_receipt", "name", "code_shusai", "generic", "form", "unit")) %>% 
  mutate(total = if_else(is.na(total_in), 0, total_in) + 
           if_else(is.na(total_out), 0, total_out) + 
           if_else(is.na(total_hos), 0, total_hos)) %>% 
  mutate(price = ifelse(!is.na(price_in), price_in, 
...
### delete
rm(list = c("df_ndb_t_2017_in", "df_ndb_t_2017_out", "df_ndb_t_2017_hos"))
...
## merge 
df_ndb_2017 <- df_ndb_o_2017 %>% 
  bind_rows(df_ndb_t_2017) %>% 
  bind_rows(df_ndb_p_2017) %>% 
  mutate(year = 2017)

## delete
rm(list = c("df_ndb_o_2017", "df_ndb_t_2017", "df_ndb_p_2017"))
